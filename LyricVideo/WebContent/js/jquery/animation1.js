...
function drawIt(ctx, currentAudioTime, lines) {
	...
	for (var i = 0; i < lines.length; i++) {
		aLine = lines[i];
		if ((aLine.startTime < (currentAudioTime + (10 * 1000)))
				&& (aLine.endTime > (currentAudioTime - (2 * 1000)))) {
			for (var j = 0; j < aLine.words.length; j++) {
				...
				aWord = aLine.words[j];
				xPosition = ((aWord.startTime - currentAudioTime) / ZOOM_LEVEL)
						+ CURRENT_TIME_POSITION;
				yPosition = yPosition + 60;
				spaceWidth = (aWord.endTime - aWord.startTime) / ZOOM_LEVEL;

				ctx.beginPath();
				ctx.globalAlpha = 0.2;
				ctx.fillStyle = BOX_FILL_COLOUR;
				ctx.strokeStyle = BOX_OUTLINE_COLOUR;
				ctx.rect(xPosition, yPosition, spaceWidth, 60);
				ctx.fill();
				ctx.lineWidth = 2;
				ctx.stroke();
